...
/* ── TITLE ─────────────────────────────────────────────────────── */
{layout:'title', kicker:'Day 5 · Pt. III', title:'Modeling the Kidney', subtitle:'What a simulation actually is, and why we trust them', meta:'Lead: Love · lecture + live board work'},

/* ── HOOK ───────────────────────────────────────────────────────── */
{title:'A model is only as good as its worst assumption', kind:'Hook', kicker:'The organizing idea for today',
 block:{type:'quote', text:'A model that makes accurate predictions for the wrong reasons is not a good model. It is a coincidence.', cite:'On why every model needs an assumption audit'}},

/* ── WHAT IS A MODEL ────────────────────────────────────────────── */
{title:'What is a model, really?', kind:'Foundation', kicker:'19.1 — Definitions',
 subtitle:"Every equation we've written so far this course was already a model. Today we make that explicit.",
 block:{type:'bullets', items:[
   ...
   'A simulation is just a model solved numerically instead of by hand — same equations, more parameters, faster answers',
 ]}},
...
{title:'Renal modeling has a 180-year head start', kind:'History', kicker:'19.2 A Brief History',
 block:{type:'timeline', steps:[
   ...
   {h:'Deen, Robertson & Brenner, 1972', p:"First ODE model of glomerular ultrafiltration — what we used on Day 3"},
   {h:'Stephenson; Kokko & Rector, 1966–72', p:"First countercurrent ODE models of the loop of Henle — what we'll use on Day 6"},
...
/* ── WHAT WE SIMULATE ───────────────────────────────────────────── */
{title:'What we will actually simulate today', kind:'Preview', kicker:'Two models, two days',
 block:{type:'two-col',
   ...
   right:{h:'The NephronSimulator',       body:['A compartment model of the full nephron — all five segments, ten adjustable parameters, live dynamics','This is the simulator we will run together today']}}},

/* ── DIVIDER ────────────────────────────────────────────────────── */
{layout:'divider', kicker:'Live simulation', title:"Let's Explore It Together", subtitle:'The simulator is already built. Today we take it apart.', tag:'Live session'},
...
{title:'Parameters 1–5: filtration and hormones', kind:'Simulator parameters', kicker:'What we built in (1 of 2)',
 block:{type:'compare', cols:['Default','What it controls'],
   rows:[
     {label:'Plasma Sodium',           vals:['140 mEq/L', 'Sets the sodium load entering the glomerulus. The entire filtration chain responds to this.']},
     {label:'Plasma Glucose',          vals:['95 mg/dL',  'Above ~180 mg/dL the Tm-limited SGLT2 is overwhelmed — glucose appears in urine.']},
     {label:'GFR',                     vals:['120 mL/min','Total filtration rate. Below 60 simulates CKD; below 15 is ESRD.']},
     {label:'ADH Level',               vals:['2.5 U',     'Controls collecting duct water permeability. ADH = 0 simulates diabetes insipidus.']},
     {label:'Aldosterone',             vals:['2.5 U',     'Scales distal tubule sodium reabsorption. High aldosterone = Na retention, hypertension.']},
   ]}},
...
{title:'Parameters 6–10: tubular mechanics', kind:'Simulator parameters', kicker:'What we built in (2 of 2)',
 block:{type:'compare', cols:['Default','What it controls'],
   rows:[
     {label:'Water Intake',            vals:['1.5 mL/min','Volume load independent of GFR — simulates drinking or IV fluids.']},
     {label:'PT Reabsorption Fraction',vals:['0.67',      'Proximal tubule reabsorbs ~67% isosmotically by default. Reducing this mimics PT injury.']},
     {label:'Loop of Henle Strength',  vals:['1.0x',      'Scales the TAL pump. Setting this to 0.2 collapses the medullary gradient — mimics furosemide.']},
     {label:'Distal Na Reabsorption',  vals:['0.09',      'Fine-tuning by the distal tubule. Aldosterone-sensitive.']},
     {label:'CD Water Permeability',   vals:['0.90',      'Direct collecting duct permeability. Interacts multiplicatively with ADH and the medullary gradient.']},
   ]}},
...
{title:'Try it: remove the collecting duct cap', kind:'Code edit', kicker:'Edit 2 of 2',
 block:{type:'code-edit',
   ...
   change:  'Remove the outer min(0.97, ...) — let it run uncapped',
